...
function findHexagramIndex(upper: string, lower: string): number {
  // 简化的映射，避免重复键
  const upperValues: { [key: string]: number } = {
    '乾': 0, '兑': 1, '离': 2, '震': 3, '巽': 4, '坎': 5, '艮': 6, '坤': 7
  };
  
  const lowerValues: { [key: string]: number } = {
    '乾': 0, '兑': 1, '离': 2, '震': 3, '巽': 4, '坎': 5, '艮': 6, '坤': 7
  };
  
  const upperValue = upperValues[upper] || 0;
  const lowerValue = lowerValues[lower] || 0;
  
  return upperValue * 8 + lowerValue;
}
